...
.Objetivos
====
Utilizando una plantilla de terraform crea 2 instancias de máquina virtuales el proveedor de servicios cloud Google Cloud.: 
...
== Platilla del proyecto terraform

Para crear las instancias, utiliza la plantilla de terraform disponible en función del proveedor que vayas a usar: 

- GCP: la encontrarás en el repositorio https://github.com/ualcnsa/terraformGoogleCloudSample. 
- Azure: la encontrarás en el repositorio https://github.com/ualcnsa/terraformNewAzureSample

En primer lugar realiza un __fork__ del repositorio plantilla, para hacer las modificaciones al mismo que sean necesarias. Después, sobre tu __fork__, modifica las variables correspondientes para usar tu proyecto en la plantilla, tal y como se describe a continuación.
...
En el caso de que optes por Azure, la plantilla Terraform para Azure crea una red virtual, un grupo de seguridad de red y dos máquinas virtuales utilizando una definición de módulo.

. **VM 1**: Preparada para Docker  
  - Usada para el despliegue de Jenkins en Docker.

. **VM 2**: Preparada para Docker, con Docker Compose, JDK 17 y NodeJS LTS instalados  
  - Usada para el despliegue de aplicaciones web.

Los pasos para la configuración son: 

. Crea un archivo `terraform.tfvars` basado en la plantilla:
...
. Obtén tus credenciales de Azure con:
...
. Edita el archivo `terraform.tfvars` para agregar tus credenciales de Azure:
...
. Asegúrate de tener una clave SSH válida en `~/.ssh/id_rsa.pub`.  
Si no tienes una, genera una nueva con:
...
. Ejecuta los comandos de Terraform directamente:
...
El comando `terraform apply` creará:

- 1 Grupo de Recursos
- 1 Red Virtual con subred
- 1 Grupo de Seguridad de Red con reglas para SSH (puerto 22), HTTP (puerto 80) y Jenkins (puerto 8080)
- 2 Instancias de máquinas virtuales con Ubuntu 22.04 LTS
- 2 Direcciones IP públicas dinámicas, con un DNS asignado.


== Ejecución de terraform

A continuación, vamos a ejecutar la plantilla de terraform para crear las instancias. Se describen los comandos y las salidas esperadas para hacerlo en GCP. Los comandos de terraform son similares para la plantilla de Azure. 
...
Este paso es muy recomendable para poder acceder a las instancias de forma más sencilla. 

=== Azure

Si estamos trabajando en Azure, nuestras máquinas ya tendrán un nombre de DNS asignado, ya que la plantilla ya está preparada para crear IPs dinámicas, que se liberan al apagar la máquina y tomas una nueva IP al encenderla. Por ese motivo, Azure permite asignar un nombre de DNS a cada instancia que no cambiará al reiniciar la máquina.

Sin embargo, en GCP tendremos que asignar un nombre de DNS a las IPs publicas. 
...
A continuación se describe cómo hacerlo para GCP con un dominio .tech, pero puedes usar cualquier otro dominio que tengas disponible.
...
=== Configuración de nombres de dominio en GCP
...
=== Configuración de nombres de dominio en Azure

En Azure, la configuración de nombres de dominio es más sencilla, ya que la plantilla Terraform ya asigna un nombre DNS a las máquinas virtuales creadas. Sin embargo, si deseas personalizar ese nombre de dominio, puedes seguir estos pasos:

1. Accede al portal de Azure y selecciona el recurso de la máquina virtual que deseas configurar.

2. En el menú lateral, selecciona *Configuración* > *Configuración de red*.

3. En la sección de *Dirección IP pública*, haz clic en el nombre de la dirección IP asociada.

4. En la página de configuración de la dirección IP pública, busca la sección *Configuración de DNS*.

5. Introduce un nombre único en el campo *Etiqueta de DNS*. Este nombre será el subdominio que se asignará a la dirección IP pública de la máquina virtual.

6. Guarda los cambios. El nombre DNS estará disponible en unos minutos y tendrá el formato `<etiqueta>.region.cloudapp.azure.com`.

Por ejemplo, si introduces `mi-servidor` como etiqueta de DNS y tu región es `eastus`, el nombre DNS será `mi-servidor.eastus.cloudapp.azure.com`.

[NOTE]
====
Si quieres configurar un nombre de dominio externo a una VM Azure tendrás que cambiar la IP pública dinámica por una estática. Esto implica gastos, y puesto que ya tenemos un DNS, no es necesario otro
====.