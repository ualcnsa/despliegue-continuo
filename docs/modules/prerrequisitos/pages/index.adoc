...
Existen distintas opciones respecto al entorno en el que realizar las actividades propuestas. Tendrás que conectar a las instancias en un proovedor de nube pública como GCP o Azure, lanzar Terraform, lanzar contenedores Docker, etc. Dependiendo del sistema operativo de tu máquina local y de tus gustos personales, te recomiendo alguna las siguientes alternativas:
...
=== Cloud Shell
Tanto Google Cloud como Azure proporcionan la herramienta *Cloud Shell* que permite trabajar cómodamente contra GCP o Azure desde el navegador web. Recuerda que, en GCP, Cloud Shell tiene una cuota de uso, con un límite de número de horas semanales (168h): 
...
A continuación se explican las herramientas que debes configurar si estás usando tu máquina local, aunque probablemente algunas ya las tendrás instaladas. Si optas por trabajar en Cloud Shell, ya estarán instaladas.
...
Azure for Students es una suscripción gratuita que proporciona crédito y servicios en la nube para estudiantes. 
Para activarla, sigue estos pasos descritos en el siguiente enlace: https://ualjjcanada.github.io/azure-tips/docs/crear-suscripcion-AzureParaEstudiantes.html[Creación de la suscripción Azure para Estudiantes].

TIP: Consulta la web https://ualjjcanada.github.io/azure-tips/ para tutoriales y consejos sobre la gestión de suscripciones Azure para Estudiantes.
...
== CLI para el proveedor de nube

Un CLI (Command Line Interface) es una interfaz de usuario que permite interactuar con un sistema operativo o software a través de comandos escritos en una terminal o consola. Es una herramienta esencial para desarrolladores y administradores de sistemas, ya que permite realizar tareas de manera eficiente y automatizada.
...
Azure CLI es una herramienta de línea de comandos que permite interactuar con los servicios de Azure. Está disponible para su instalación https://docs.microsoft.com/en-us/cli/azure/install-azure-cli-windows?view=azure-cli-latest[en esta página].

Sigue los pasos descritos en https://ualjjcanada.github.io/azure-tips/docs/azure-cli.html[este enlace] para instalar Azure CLI en tu máquina local y comenzar a trabajar con Azure CLI: 
