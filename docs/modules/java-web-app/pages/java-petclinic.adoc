...
Para ejecutar el proyecto PetClinic, en tu máquina de desarrollo local puedes construir el `.jar` (empaquetado) y ejecutarlo:
[IMPORTANT]
====
...
[NOTE]
====
Para que la construcción ejecute correctamente todos los tests debes tener docker instalado y funcionando, ya que en el proceso descarga los contenedores de MySQL y PostgreSQL para ejecutar los tests de integración. Si no tienes docker, puedes saltarte los tests de integración con el comando `./mvnw package -Dtest=!PostgresIntegrationTests*`.
====

.Página principal de PetClinic
image::petclinic-homepage.png[role="thumb", align="center"]

En su configuración predeterminada, Petclinic utiliza una base de datos en memoria (http://www.h2database.com/html/main.html[H2]) que se inicia con datos predeterminados, y los nuevos datos que se guarden se pierden al reiniciar la aplicación. 
...
El archivo `docker-compose.yml` que permite iniciar MySQL o PostgreSQL puedes consultarlo en la carpeta raíz del proyecto.
...
image::vscode-devcontainer-banner.png[role="thumb", align="center", width="60%"]

Pero si aceptas, se descargará una imagen de Docker con un entorno de desarrollo preconfigurado para trabajar con el proyecto, así como las imágenes de MySQL y PostgreSQL . En ese caso, al lanzar la construcción del proyecto, los tests de integración se ejecutan sobre las bases de datos MySQL y Postgres.
====
...
<1> agente o nodo de Jenkins en que ejecuta la construcción del proyecto. En el ejemplo, `any` indica que se ejecutará cualquier nodo, en nuestro caso será en el nodo _principal_ (_main_) ya que es el único que hay definido.
<2> como herramienta para la construcción se usará Naven. Pon aquí el *nombre* que diste a tu instalación de Maven configurada previamente en _Tools Configuration_. 
<3> Bloque de `stages`: fases o etapas que conforman el pipeline
<4> Fase de descarga del repositorio git
<5> Fase de compilación, ejecución de test y empaquetado de la aplicación. Se realiza con los _goals_ de Maven `clean package`: primero `clean` elimina todo lo generado en la construcción anterior que se encuentra en la carpeta `target`, y a continuación se lanza la construcción con `package` tal y como está definida en el archivo `pom.xml`. La opción `-Dcheckstyle.skip` anula el análisis de CheckStyle, que lo añadiremos en una fase posterior.
...
[NOTE]
====
Alternativamente, podemos hacer que la construcción se realice en un agente que sea un contenedor docker. Para ello, crea el siguiente pipeline de nombre `spring-petclinic-pipeline-agent-docker`:
[source,groovy]
----
pipeline {
   agent {
...
----
<1> Agente donde construir el proyecto, ahora no se hace en el nodo principal sino en un contenedor docker seleccionado. La imagen de Maven que se utiliza es `maven:3.9-eclipse-temurin-21-alpine`, y se monta el volumen del directorio `.m2` del usuario en el contenedor. Se ha elegido la imagen de Maven con Java 21 y Alpine, que es una distribución de Linux muy ligera.
====
...
****
La https://es.wikipedia.org/wiki/Cobertura_de_c%C3%B3digo[*Cobertura de código*] nos indica el porcentaje de código de producción que está siendo ejecutado por los test. Es deseable tener un valor de cobertura lo más próximo posible al 100%
****
...
Para ejecutar y visualizar el *informe de CheckStyle en Jenkins*: 
