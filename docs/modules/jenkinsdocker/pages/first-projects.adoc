...
----
[Pipeline] sh
+ docker inspect -f . maven:3.9-eclipse-temurin-21-alpine

permission denied while trying to connect to the Docker daemon socket at unix:///var/run/docker.sock: Get "http://%2Fvar%2Frun%2Fdocker.sock/v1.48/containers/maven:3.9-eclipse-temurin-21-alpine/json": dial unix /var/run/docker.sock: connect: permission denied
----
...
[NOTE]
====
Más info en: https://www.jenkins.io/doc/book/pipeline/docker/#using-multiple-containers[Using multiple containers].
====
...
. Conecta por SSH a la máquina Jenkins: `ssh ubuntu@__instancia-jenkins__` o bien `ssh azureuser@__instancia-jenkins__` si estás trabajando con Azure.

.Conexión SSH a la instancia Jenkins
image::ssh-from-developer-to-jenkins.png[role="thumb", align="center"]

[start=2]
. Crea la carpeta donde se va a guardar la nueva pareja de claves: `mkdir ~/jenkins_home/.ssh` y muevete a ella con `cd ~/jenkins_home/.ssh`.
...
[source,bash,subs="verbatim,quotes"]
----
cat ~/jenkins_home/.ssh/id_rsa_deploy.pub
----
...
[NOTE]
====
Si no ha sido correcto, verifica que la ruta al archivo de la clave privada es correcta, y que el nombre de la máquina de despliegue es correcto. 
====
